...
key = 'q6Q0VYtIlndS4IA'
secret = 'QoFOK9U6TJnCOBe3I3HGlVlmm'
app = 'thanapon1195'
...
def subscription(topic,msg):
    if msg == "b'?'":
        microgear.publish("/gearname/Count",countPeople)
...
def checkmark (Frame,frameSize,sizeLong,OffsetY,OffsetX) :
    ...
    (cnts, _) = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0]
